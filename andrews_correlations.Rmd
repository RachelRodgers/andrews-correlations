...
MakeCorrelationMatrix <- function(abdTable, bacteria, phage, sig_p = 0.05) {
  # Generate a correlation matrix from an abundance data frame
  #
  # Args:
...
physeqBacteria <- readRDS("psm.DS1.RDS")
sampleDataBacteria <- as(sample_data(physeqBacteria), "data.frame")
physeqBacteria # 775 x 95

physeqPhage <- readRDS("psm.DS1.phage.RDS") # currently contains phage + other stuff
sampleDataPhage <- as(sample_data(physeqPhage), "data.frame")
physeqPhage # 1135 x 84
...
seroYesSamples <- unique(c(bacteriaSeroYes, phageSeroYes)) # 37 samples
seroNoSamples <- unique(c(bacteriaSeroNo, phageSeroNo)) # 63 samples
```
...
numMissingBacterialRanks[ranks] # 139 NAs in Genus column

taxTablePhage <- as.data.frame(as(tax_table(physeqPhage), "matrix"))
numMissingPhageRanks <- map_int(taxTablePhage, ~ sum(is.na(.)))
numMissingPhageRanks[ranks] # 19 NAs in Order, 12 NAs in Family, 29 NAs in Genus
```

The bacterial data has 139 NA's in the Genus column.  The phage data has 19 NA's in Order, 12 NA's in Family, and 29 NA's in the Genus column.  We need to fill in these missing taxonomy names before proceeding so correlation matrices are more informative.  After correcting missing taxonomy, we can proceed to modifying the data to generate matrices of abundances for correlation tests.

```{r get-bacterial-data}
# Adjust taxonomy
physeqBacteriaModified <- FillEmptyRanks(physeqBacteria) # 775 x 95
...
```{r get-phage-data}
# Adjust taxonomy
physeqPhageModified <- FillEmptyRanks(physeqPhage) # 1135 x 84
